...
<div id="right-1" class="R-mainbox1 mar-b20">
	...
	<div class="top-list">
		...
		<div id="top-list-2" style="display: none;">
			<ul class="wpp-list item">
				<?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=e0786e39252051e8ae3a228167a777d7&action=sitehits&siteid=1&num=10&order=weekviews+DESC&cache=60\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">修改</a>";}$tag_cache_name = md5(implode('&',array('siteid'=>'1','order'=>'weekviews DESC',)).'e0786e39252051e8ae3a228167a777d7');if(!$data = tpl_cache($tag_cache_name,60)){$content_tag = pc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'sitehits')) {$data = $content_tag->sitehits(array('siteid'=>'1','order'=>'weekviews DESC','limit'=>'10',));}if(!empty($data)){setcache($tag_cache_name, $data, 'tpl_data');}}?>
				<?php $n=1;if(is_array($data)) foreach($data AS $v) { ?>
				<li class="title-li">
		...
		<div id="top-list-3" style="display: none;">
			<ul class="wpp-list item">
				<?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=13c597aa4af8d6d6cb19a4654397280f&action=sitehits&siteid=1&num=10&order=monthviews+DESC&cache=60\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">修改</a>";}$tag_cache_name = md5(implode('&',array('siteid'=>'1','order'=>'monthviews DESC',)).'13c597aa4af8d6d6cb19a4654397280f');if(!$data = tpl_cache($tag_cache_name,60)){$content_tag = pc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'sitehits')) {$data = $content_tag->sitehits(array('siteid'=>'1','order'=>'monthviews DESC','limit'=>'10',));}if(!empty($data)){setcache($tag_cache_name, $data, 'tpl_data');}}?>
				<?php $n=1;if(is_array($data)) foreach($data AS $v) { ?>
				<li class="title-li">
					<?php if($n==1 or $n==2 or $n==3) { ?><i class="s-number"><?php } else { ?><i class="q-number"><?php } ?><?php echo $n?></i>
					<a href="<?php echo $v['url'];?>" title="<?php echo $v['title'];?>" class="wpp-post-title" target="_blank" ><?php echo str_cut($v['title'],60,'...');?></a>						
...
<!-- 右侧广告 -->
<div id="right-ad01" class="R-mainbox1 mar-b20">
	<?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"block\" data=\"op=block&tag_md5=9fbd7c8f22431817195cd5994bbebc02&pos=rightside_1\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">添加碎片</a>";}$block_tag = pc_base::load_app_class('block_tag', 'block');echo $block_tag->pc_tag(array('pos'=>'rightside_1',));?><?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
</div>
<!-- 右侧广告结束 -->
...
<div id="right-ad02" class="R-mainbox1 mar-b20">
	<?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"block\" data=\"op=block&tag_md5=ea5f2911f44098a780e05ecdf5b3d54a&pos=rightside_2\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">添加碎片</a>";}$block_tag = pc_base::load_app_class('block_tag', 'block');echo $block_tag->pc_tag(array('pos'=>'rightside_2',));?><?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
</div>
