function flashupload(uploadid, name, textareaid, funcName, args, module, catid, authkey, lang) {
	var args = args ? '&args=' + args : '';
	var setting = '&module=' + module + '&catid=' + catid + '&authkey=' + authkey;
	var lang = lang ? '&lang='+lang : '';
	var okVal = lang==''||lang=='zh_cn' ? '确定' : 'Ok';
	var cancelVal = lang==''||lang=='zh_cn' ? '取消' : 'Cancel';	
	window.top.art.dialog({
		title : name,
		id : uploadid,
		okVal:okVal,
		cancelVal:cancelVal,
		iframe : '/index.php?m=attachment&c=attachments&a=swfupload&rand='+Math.random() + args + setting + lang,
		width : '500',
		height : '420'
	}, function () {
		if (funcName) {
...
function submit_ckeditor(uploadid, textareaid) {
	var d = window.top.art.dialog.data(uploadid);
...
function submit_images(uploadid, returnid) {
	...
	IsImg(in_content[0]) ? (returnid.jquery ? returnid.val(in_content[0]) : $('#' + returnid).attr("value", in_content[0])) : alert('选择的类型必须为图片类型');
}
...
function insert2editor(id, in_content, del_content) {
	...
	$.get("http://js.cyzone.cn/index.php?m=attachment&c=attachments&a=swfdelete", {
		data : del_content
	}, function (data) {});
