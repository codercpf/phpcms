...
////		echo $arr_touxiang;exit();
	...
	public function register() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$userinfo['username'] = (isset($_POST['userAccount']) && is_email($_POST['userAccount'])) ? $_POST['userAccount'] : exit('0');
			$userinfo['nickname'] = (isset($_POST['nickname']) && is_username($_POST['nickname'])) ? $_POST['nickname'] : '';
			
			$userinfo['email'] 	  = (isset($_POST['userAccount']) && is_email($_POST['userAccount'])) ? $_POST['userAccount'] : exit('0');
			$userinfo['password'] = isset($_POST['userPass'])  ? $_POST['userPass'] : exit('0');	
...
//			}
	...
	public function account_manage_info2() {
		if(isset($_POST['dosubmit'])) {
			//更新用户昵称
...
/*
//到达这一步说明已经收到验证邮件，发送成功，故除去mail配置
		$email_config = getcache('common', 'commons');
...
*/
	...
	public function public_updatepassword(){
		...
		if($_GET['code'])
		{
			...
			$memberinfo = $this->db->get_one(array('userid'=>$code[0]));  //根据用户ID获取用户完整信息
...
//此处设置新密码
			...
			showmessage(L('operation_success').L('newpassword').':'.$password, 'index.php?m=member&c=index&a=login', '3000');
		}else{
			showmessage(L('operation_failure'), 'index.php?m=member&c=index&a=login');
		}
	}
