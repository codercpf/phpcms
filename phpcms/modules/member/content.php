...
/*
		echo"<pre>";
		print_r($grouplist[$memberinfo['groupid']]);
		echo"<pre>";
		exit();
*/

//		echo $grouplist[$memberinfo['groupid']]['allowpost'];exit();
...
/*
			$arr=array('catid'=>$catid, 'roleid'=>$memberinfo['groupid'], 'is_admin'=>0, 'action'=>'add');
			echo"<pre>";
			print_r($arr);
			echo"<pre>";
			exit();
...
//去掉栏目id	-------------------------------------------------------------------------------这里添加栏目id，使之显示
//			unset($forminfos['catid']);
			$workflowid = $setting['workflowid'];
			header("Cache-control: private");
			$template = $MODEL['member_add_template'] ? $MODEL['member_add_template'] : 'content_publish_project';
